...
function EditMae({ onNext, onBack, formData = {}, onChange }) {
	...
	function resetFormFields() {
		if (!formData.nomeMae) onChange({ target: { name: "nomeMae", value: "" } });
		if (!formData.nascimentoMae)
			onChange({ target: { name: "nascimentoMae", value: "" } });
		if (!formData.enderecoMae)
			onChange({ target: { name: "enderecoMae", value: "" } });
		if (!formData.numeroCasaMae)
			onChange({ target: { name: "numeroCasaMae", value: "" } });
		if (!formData.cepMae) onChange({ target: { name: "cepMae", value: "" } });
		if (!formData.cpfMae) onChange({ target: { name: "cpfMae", value: "" } });
		if (!formData.rgMae) onChange({ target: { name: "rgMae", value: "" } });
		if (!formData.telefoneMae)
			onChange({ target: { name: "telefoneMae", value: "" } });
		if (!formData.emailMae)
			onChange({ target: { name: "emailMae", value: "" } });
		if (!formData.profissaoMae)
			onChange({ target: { name: "profissaoMae", value: "" } });
		if (!formData.trabalhoMae)
			onChange({ target: { name: "trabalhoMae", value: "" } });
		if (!formData.telefoneTrabalhoMae)
